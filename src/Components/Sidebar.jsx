...
export default function Sidebar() {
  ...
  return (
    <div className='w-1/3 md:w-1/6 bg-[#212121] px-5 overflow-auto pb-8 h-screen'>
        <ul className='flex flex-col border-b  border-gray-600 py-2'>
            {mainLinks.map(({icon, title}) => {
                return(
                    <li key={title}>
                        <a href="#" className="flex items-center gap-4 pl-2 py-3 hover:bg-zinc-600 rounded-xl">{icon}<span className='text-sm tracking-wider'>{title}</span></a>
                    </li>
                )
            })}
        </ul>
